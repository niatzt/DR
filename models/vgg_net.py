...
#channel_mean = np.array([123.68, 116.779, 103.939], dtype=np.float32) #rgb
channel_mean = np.array([105.313, 74.007, 52.073], dtype=np.float32) #rgb
...
def vgg_fc8(input_batch, name, apply_dropout, output_dim=5):
...
def vgg_prob(input_batch, name, apply_dropout, output_dim=5):
    fc7 = vgg_fc7(input_batch, name, apply_dropout)
    with tf.variable_scope(name):
        # layer 8 (no ReLU after fc8)
        fc8 = fc('fc8', fc7, output_dim=output_dim)
